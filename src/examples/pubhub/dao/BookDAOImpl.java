...
public class BookDAOImpl implements BookDAO {
	...
	@Override
	public List<Book> getAllBooks_Tags() {
		
		List<Book> books_tags = new ArrayList<>();

		try {
			connection = DAOUtilities.getConnection();	// Get our database connection from the manager
			String sql = "SELECT * FROM Books LEFT JOIN Book_tags ON books.isbn_13 = book_tags.isbn_13;";
			stmt = connection.prepareStatement(sql);	// Creates the prepared statement from the query
		...
		// return the list of Book and Tag objects populated by the DB.
		return books_tags;
